...
function detectRocketXY(){
	for (var i = 0; i < arm.arrayClassArm.length; i++) {
		for(let j = 0;j < monster.arrayMonster.length;j++){
			if(arm.arrayClassArm[i].posLy <= monster.coordinateMy[j] + 50 && arm.arrayClassArm[i].posLx + 10 > monster.coordinateMx[j] && arm.arrayClassArm[i].posLx + 20 < monster.coordinateMx[j]+70){
